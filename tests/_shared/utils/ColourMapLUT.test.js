...
// ColourMapLUT.buildLUT/sampleColour call the bare global `constrain()`, which in
// production only exists because p5.min.js defines it as a global. This file has an
// implicit, untested dependency on p5 (confirmed via grep: no local definition anywhere
// in authored code) — flagged as a Stage 2 backlog item. We stub it locally here so
// these tests can run without loading p5.
let previousConstrain;

before(() => {
  previousConstrain = globalThis.constrain;
  globalThis.constrain = (n, low, high) => Math.min(Math.max(n, low), high);
});

after(() => {
  globalThis.constrain = previousConstrain;
});
...
describe("ColourMapLUT.buildLUT", () => {
  test("GREYSCALE is linear: lut value at index i equals i, at the low/mid/high boundaries", () => {
    ...
    ColourMapLUT.buildLUT(ColourMapLUT.GREYSCALE, lut, null, false);
  ...
  test("a falsy colourMapData is a no-op (lut left untouched)", () => {
    ...
    ColourMapLUT.buildLUT(null, lut, null, false);
  ...
  test("packed LUT assembles r|g<<8|b<<16|(255<<24) per entry when little-endian", () => {
    ...
    ColourMapLUT.buildLUT(ColourMapLUT.GREYSCALE, lut, lutPacked, true);
  ...
  test("packed LUT is left untouched when isLittleEndian is false", () => {
    ...
    ColourMapLUT.buildLUT(ColourMapLUT.GREYSCALE, lut, lutPacked, false);
...
describe("ColourMapLUT.sampleColour", () => {
  test("a falsy colourMapData returns opaque white", () => {
    assert.deepEqual(ColourMapLUT.sampleColour(null, 0.5), [255, 255, 255]);
  });

  test("GREYSCALE sampleColour matches buildLUT at t = 0, 0.5, 1", () => {
    assert.deepEqual(ColourMapLUT.sampleColour(ColourMapLUT.GREYSCALE, 0), [0, 0, 0]);
    assert.deepEqual(ColourMapLUT.sampleColour(ColourMapLUT.GREYSCALE, 0.5), [128, 128, 128]);
    assert.deepEqual(ColourMapLUT.sampleColour(ColourMapLUT.GREYSCALE, 1), [255, 255, 255]);
  });

  test("t outside [0, 1] is clamped before sampling", () => {
    assert.deepEqual(ColourMapLUT.sampleColour(ColourMapLUT.GREYSCALE, -5), [0, 0, 0]);
    assert.deepEqual(ColourMapLUT.sampleColour(ColourMapLUT.GREYSCALE, 5), [255, 255, 255]);
  });
});
...
describe("ColourMapLUT.valueToColour", () => {
  ...
  test("val outside [0, 1] clamps to the first/last LUT entry without touching the global constrain", () => {
    const lut = new Uint8Array(256 * 3);
    ColourMapLUT.buildLUT(ColourMapLUT.GREYSCALE, lut, null, false);
